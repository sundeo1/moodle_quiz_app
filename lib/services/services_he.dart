...
Future processQuiz() async {
  var domainname = 'http://sundeo.pagekite.me';
...
Future<Quiz> getAttemptData(attemptID) async {
  var domainname = 'http://sundeo.pagekite.me';
  var serverurl = domainname + '/webservice/rest/server.php';
  // var domainname = 'http://10.0.2.2';
  //var domainname = 'http://41.210.154.80';
  ...
  var attemptid = attemptID.toString();
  var page = '0';
  var arguments = '&attemptid=' + attemptid + '&page=' + page;
  ...
  if (response.statusCode == 200) {
    ...
    //print("Error connecting to API");
    throw Exception('Failed to load attempt data');
}
...
Future getQuizzesByCourses() async {
...
Future startAttempt(quizID) async {
  var domainname = 'http://sundeo.pagekite.me';
  var serverurl = domainname + '/webservice/rest/server.php';
  var functionname = 'mod_quiz_start_attempt';
  var quizid = quizID.toString();
  //var preflightdata = ''; //Optional
  var forcenew = '1'; //Whether to force a new attempt or not. Optional
  var arguments = '&quizid=' + quizid + '&forcenew=' + forcenew;
  ...
  if (response.statusCode == 200) {
    var data = jsonDecode(response.body);
    var attemptID = data["attempt"]["id"];
    print(attemptID);
    //getAttemptData(attemptID);
    //return Quiz.fromJson(jsonDecode(response.body));
    //return response.body;
    return getAttemptData(attemptID);
    //print(decodedData);
  } else {
    print("Error connecting to API");
  }
